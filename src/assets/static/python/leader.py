...
id= sys.argv[2]
theme_id= sys.argv[6]
organization_id= sys.argv[7]
location_id= sys.argv[8]
person_id= sys.argv[9]

print theme_id

theme= open(path.join(theme_id)).read()
organization = open(path.join(organization_id)).read()
location = open(path.join(location_id)).read()
person = open(path.join(person_id)).read()
...
# store to file
d2=sys.argv[3]
wc.to_file(path.join(d2, id+"_wordcloud.jpg"))
