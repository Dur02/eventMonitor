...
class WordCloud(object):
    ...
    def __init__(self, font_path=None,img_path="", width=400, height=200, margin=2,
    ...
    def to_image(self):
        ...
        for i in range(0,width):
            for j in range(0,height):
                if im_array[i,j]<255 and im_array[i,j]>0:
                    im_array[i,j] = 0
                if im_array[i,j] == 0:
                    im_array[i,j] = 235
        im2=im.convert(self.mode)###
        img = im2.resize((width * self.scale, height * self.scale))
