(function() {
  ...
  planet.loadPlugin(planetaryjs.plugins.earth({
    topojson: { file:   '/static/Visualization/json/world-110m.json' },
    oceans:   { fill:   '#001320' },
    land:     { fill:   '#06304e' },
  ...
  planet.projection.rotate([200, -10, 0]);
  planet.draw(canvas);
  ...
  d3.json('/static/Visualization/json/year_quakes_small.json', function(err, data) {
    if (err) {
    ...
    var updateDate = function() {
      d3.select('#date').text(moment(currentTime).utc().format("YYYY年MM月DD日 HH时mm分"));
    };
  ...
  function autocenter(options) {
    ...
    var div_height=document.getElementById('globe_canvase').offsetHeight;
      var div_width=document.getElementById('globe_canvase').offsetWidth;
