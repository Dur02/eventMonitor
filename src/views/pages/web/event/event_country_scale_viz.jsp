...
<body>
	<div class="index_top clearfix">
		<h3 class="fl title">事件库分析 > 国家稳定分析</h3>
             ...
             <input type="hidden" id="moduleCode" value="${moduleCode}">
             	<div>
             		<select class="selectCounts" style="height:28px; border: 1px solid #ccc;"  id="selectPXFS">
             			<option value="1">按总稳定值排序</option>
             			<option value="2">按国家名称排序</option>
             			<option value="3">按平均稳定值排序</option>
             		</select>
...
<script type="text/javascript">
		 ...
		 function drawChart(n){
        	...
        	for(var i in lineData){
				if(country_name==lineData[i].chineseName){
					...
					line_bar_html +="<div class='col-md-12 bor' style='margin-top: 12px;' id='bigDiv"+country_name+"'>"+
										"<div ></div>"+
										"<div class='col-md-1'>"+
											"<span style='display:inline-block;width:20px;padding-top:150px;margin-left: 65px;font-size: 15px;'>"+country_name+"</span>"+
											"</div>"+
											"<div class='col-md-11' style='height:400px;'>"+
												"<div id='line_eachart_scale"+country_name+"' class='amcharts'></div>"+
										"</div>"+
									"</div>";
					$("#line_bar_data_html").append(line_bar_html);				
					init_line_chart_scale(series_data2,country_name);
				}
			}
		}
	...
	function render(configId){
		startLoad();
		$.ajax({
			url:'<%=basePath%>/search/event_country_scale_viz/'+configId,
			type: 'POST',
			dataType:"json",
			async:false,
	...
	function renderData(data){
		...
		series_data2 = [];
		var data_reult;
		data_reult = data.country_scale_dicBySumScale;
			...
			for(var item_two in data_reult[item_one]){
				 data_html += "<li class='li_item clearfix'>"+
									...
									"<div class='li_item_r clearfix' style='border-right:1px solid #ddd;'>"+
									    "<p class='li_item_p'>总稳定值</p>"+
										"<p title='"+data_reult[item_one][item_two].SumScale+"'>"+data_reult[item_one][item_two].SumScale+"</p>"+
									"</div>"+"<div class='li_item_r clearfix'>"+
									    "<p class='li_item_p'>平均稳定值</p>"+
										"<p>"+data_reult[item_one][item_two].AvgScale+"</p>"+
									"</div>"+
								"</li>"
			}
			data_html+="</ul>"
		...
		$("#selectPXFS").change(function () {
			...
			if($("#selectPXFS option:selected").val()==2){
				data_reult = data.country_scale_dicByCode;
			}
			//按照平均稳定值排序
			if($("#selectPXFS option:selected").val()==3){
				data_reult = data.country_scale_dicByAvgScale;
			}
			//加载国家显示列表 
			for(var item_one in data_reult){
				var data_html ="<ul>";
