...
<body>
	...
	<div class="index_top clearfix">
		<h3 class="fl title">图谱库分析 >组织分析</h3>
...
<script type="text/javascript">
	...
	function renderData(data){
		...
		//组织分析中组织个数、权重值Top3对应的详细值
		/* var orgAnalyDetils = {
							"20180102":[{"key":"组织1","value":222},{"key":"组织2","value":22},{"key":"组织3","value":223}],
							"20180103":[{"key":"组织1","value":222},{"key":"组织2","value":22},{"key":"组织3","value":223}]
							}; */
		...
		for(var i=0;i<num;i++){
			$('.drop-down-content').append(' <li value="'+organStatisData[i].key+'" class="li_1 li">组织：'+organStatisData[i].key+'    组织（权重）：'+organStatisData[i].value+'</li>');
		}
		//下拉框点击时间
		...
		for(var i = 0,j = orgValueTop3.length;i < j;i++){
			$('.drop-down-content_analysis').append(' <li  data-date="'+orgValueTop3[i].date+'" data-type="value" class="li_1_analysis li">组织（权重）：'+orgValueTop3[i].value +'&nbsp;&nbsp;&nbsp;    时间：'+orgValueTop3[i].date+'</li>');
		}
		for(var i = 0,j = orgCountTop3.length;i < j;i++){
			$('.drop-down-content_analysis').append(' <li data-date="'+orgCountTop3[i].date+'" data-type="count" class="li_1_analysis li">组织（去重）：'+orgCountTop3[i].value +'&nbsp;&nbsp;&nbsp;    时间：'+orgCountTop3[i].date+'</li>');
		}
		//下拉框点击
	...
	//渲染 时间组织分析图
	//组织分析数据、组织个数Top3的集合、组织个组织（权重）Top3的集合、组织详情信息集合
			        	...
			        	if(params.length >1){
			        		if(orgAnalyDetils[date] != undefined){
					        	 tooltipHtml+='<div style="width: 388px;height: 336px;background: url(<%=basePath %>/static/images/kuang1.png);position: relative;">'
								            ...
								            +'<span style="color: #11c0dd;margin-left: 5px;">组织（去重）</span>'
								            ...
								            +'<span style="color: #11c0dd;margin-left: 5px;">组织（权重）</span>'
				        		...
				        		tooltipHtml+='时间：'+params[0].name+'<br/>组织（去重）：'+params[0].value+'个<br/> 数 量：'+params[1].value+'个次';
				        	}
			        	}else{
			        		if(orgAnalyDetils[date] != undefined){
					        	 tooltipHtml+='<div style="width: 388px;height: 336px;background: url(<%=basePath %>/static/images/kuang1.png);position: relative;">'
				        		...
				        		tooltipHtml+='时间：'+params[0].name+'<br/>组织（去重）：'+params[0].value+'个<br/> 数 量：'+params[0].value+'个次';
				        	}
			        	}
	...
	function initOrgDetil(data){
		...
		//var orgArray = ['组织1', '组织2','组织3','组织4','组织5','组织6','组织7'];
