...
<%--									<option value="" selected="selected">全部</option>--%>
<%--									&lt;%&ndash; <c:forEach items="${scheduleStateList}" var="scheduleState" varStatus="indx"> &ndash;%&gt;--%>
...
<%--			trs += '<img title="强制停止" class="set" onclick="stopTask('+ item.configId+')" src="<%=basePath %>/static/images/stopTask.png"/></td>';--%>
<%--		}else{																																	--%>
<%--			&lt;%&ndash; trs += '<td><img title="执行" class="set tijiao'+item.configId+'" src="<%=basePath %>/static/images/'+exec2+'.png" onclick="updateConfigStatus('+ item.configId +')"/>'; &ndash;%&gt;--%>
...
<%--</body>--%>
<body>
<iframe src="http://219.130.114.4:13311/eventQuery" style="height: 100vh; width: 100vw; border: 0;overflow-x: hidden; overflow-y: hidden;">
